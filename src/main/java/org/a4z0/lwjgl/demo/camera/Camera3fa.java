...
public class Camera3fa<T extends Camera3fa<T>> implements Camera3fc<T> {
    ...
    protected final Matrix4f projectionView = new Matrix4f();
    ...
    */
    ...
    public Camera3fa(float x, float y, float z, float yaw, float pitch, float roll, int width, int height, float fov) {
        this.position(x, y, z).rotation(yaw, pitch, roll).dimension(width, height).setFOV(fov);
    }
    ...
    @Override
    public T rotation(float yaw, float pitch, float roll) {
        this.yaw = yaw;
        this.pitch = pitch;
        this.roll = roll;
    ...
    @Override
    public T dimension(int width, int height) {
        this.width = width;
        this.height = height;
